...
export const SubscriptionRequests: React.FunctionComponent = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => {
        return subscriptionPlanFriendlyName(record.plan_type, true);
      },
      filters: [
        {
          text: "Flex Starter",
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const orgUsers = await getData<{ result }>(`${API_URL}/subscriptions/list_upgrade_requests`, accessToken, signal)
        .then(({ result }) => {
